...
class Memory(nn.Module):
    ...
    def get_score(self, mem, query):  # keys  query
        bs, h, w, d = query.size()
        m, d = mem.size()

        device = 'cuda'

        score = torch.matmul(query, torch.t(mem)).to(device)  # b X h X w X m
        score = score.view(bs * h * w, m).to(device)  # (b X h X w) X m
    ...
    def forward(self, query, keys, train=True):
        ...
        query = query.permute(0, 2, 3, 1)  # b X h X w X d  torch.Size([1, 30, 45, 512])
    ...
    def gather_loss(self, query, keys, train):
        ...
        if train:
            ...
            softmax_score_query = softmax_score_query.to(device)
            softmax_score_memory = softmax_score_memory.to(device)
            query_reshape = query.contiguous().view(batch_size * h * w, dims)
