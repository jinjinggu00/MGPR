...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, skip3_fea):
        tensorConv1 = self.moduleConv1(x)
        tensorPool1 = self.modulePool1(tensorConv1)

        tensorConv2 = self.moduleConv2(tensorPool1)
        # tensorConv2 = tensorConv2 + skip_flow
        tensorPool2 = self.modulePool2(tensorConv2)

        tensorConv3 = self.moduleConv3(tensorPool2)
        tensorConv3 = tensorConv3  + skip3_fea
...
class Decoder(torch.nn.Module):
    def __init__(self, t_length=5, n_channel=3):
        ...
        self.memory_hf2vad = Memory_hf2vad(num_slots=1400, slot_dim=691200,
        ...
        self.moduleDeconv3 = Basic(384, 256)
    ...
    def forward(self, x, skip1, skip2, skip3):
        ...
        # tensorConv = tensorConv + tensorConv_flow

        tensorUpsample4 = self.moduleUpsample4(tensorConv)
        # tensorUpsample4 = tensorUpsample4 + tensorUpsample4_flow
...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys,  feas, skip3_fea, update_fea_flow,   train=True):

        fea, skip1, skip2, skip3 = self.encoder(x, skip3_fea)
        # print('frame fea shape is ', fea.shape)
        if train:
            updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss = self.memory(fea, keys, train)
            output, att_weight = self.decoder(updated_fea, skip1, skip2, skip3)
        ...
        # test
            ...
            updated_fea, keys, softmax_score_query, softmax_score_memory, query, top1_keys, keys_ind, compactness_loss = self.memory(
                fea, keys, train)
            updated_fea_in = updated_fea + update_fea_flow

            output, att_weight = self.decoder(updated_fea_in, skip1, skip2, skip3)
