...
def tensor_to_list(ssim_tensor_list):
    return [t.tolist() for t in ssim_tensor_list]

def score_sum(list1, list2, list3, alpha):
    list_result = []
    for i in range(len(list1)):
        list_result.append(3.1 * list1[i] + (1 - alpha) * list2[i] + 7*list3[i] )  # add the list3 into it
    print('result is :',len(list_result) ,type(list_result),list_result)
    return list_result
