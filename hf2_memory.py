...
class Memory_hf2vad(nn.Module):
    def __init__(self, num_slots, slot_dim, shrink_thres=0.0005):
        super(Memory_hf2vad, self).__init__()
        self.num_slots = num_slots
        self.slot_dim = slot_dim

        self.memMatrix = nn.Parameter(torch.empty(num_slots, slot_dim))  # M,C  2000, slot_dim = features_root*2*
    ...
    def forward(self, x):
        ...
        if self.shrink_thres > 0:
            ...
            # print('att_weight 2 is', att_weight)
...
class ML_MemAE_SC(nn.Module):
    def __init__(self, num_in_ch, seq_len, features_root,
        ...
        self.num_slots = num_slots
        self.shrink_thres = shrink_thres
        self.mem_usage = mem_usage   # [ False, True, True, True ]
        self.num_mem = sum(mem_usage)
        self.skip_ops = skip_ops      # [ "none", "concat", "concat" ]
        ...
        # memory modules
        self.mem1 = Memory_hf2vad(num_slots=self.num_slots, slot_dim=features_root * 2 * 16 * 16,
                           shrink_thres=self.shrink_thres) if self.mem_usage[1] else None
        self.mem2 = Memory_hf2vad(num_slots=self.num_slots, slot_dim=features_root * 4 * 8 * 8,
                           shrink_thres=self.shrink_thres) if self.mem_usage[2] else None
        self.mem3 = Memory_hf2vad(num_slots=self.num_slots, slot_dim=features_root * 8 * 4 * 4,
    ...
    def forward(self, x):
        ...
        if self.mem_usage[3]:  # [ False, True, True, True ]
            ...
            # unflatten
            x3 = x3.view(bs, C, H, W)
            print('the shape of the input(output) for mem3 is : ', x3.shape)

        recon = self.up_3(x3, x2 if self.skip_ops[-1] != "none" else None)

        if self.mem_usage[2]:  # [ False, True, True, True ]
            ...
            print('the shape of the input(output) for mem2 is : ', recon.shape)

        recon = self.up_2(recon, x1 if self.skip_ops[-2] != "none" else None)

        if self.mem_usage[1]:  # [ False, True, True, True ]
            # pass through memory again
            bs, C, H, W = recon.shape
            recon = recon.view(bs, -1)
            ...
            recon = recon.view(bs, C, H, W)
            print('the shape of the input(output) for mem1 is : ', recon.shape)
